...
int main(void)
{
    ...
    // Infinite loop
    while (1) {
        if (new_sensor_data == 1) {         // Chyba ve cteni senzoru, neni splnena podminka

            // Temperatura
            uart_puts("Teplota vzduchu");
            uart_puts("\t");
            itoa(dth12.temp_air, string, 10);
            uart_puts(string);
            uart_puts(" °C");
            uart_puts(" \r\n");
...
{   
    ...
    if (n_ovfs >= 3) {
        ...
        if (twi_write((SENSOR_ADR<<1) | TWI_WRITE) == 0) {
            ...
            dth12.temp_air = twi_read(TWI_ACK);
            //twi_read(TWI_ACK);
            dth12.hum_air = twi_read(TWI_ACK);
            twi_stop();
